...
ITSQ_MPL_BACKEND='WXAgg'                                ## force matplotlib backend None (auto) or one of 'GTK3Agg', 'MacOSX', 'Qt4Agg', 'Qt5Agg', 'TkAgg', 'WXAgg'
...
IV_Resistance_m100pA="[f.resistance for f in self.frames if f.current==-100][0] / cfg.OUTPUT_OHM_SCALE" ## any value which is not all UPPERCASE is interpreted as an extra measurement
IV_Resistance_m200pA="[f.resistance for f in self.frames if f.current==-200][0] / cfg.OUTPUT_OHM_SCALE" ## any value which is not all UPPERCASE is interpreted as an extra measurement
...
AHP_CHECK_NONE=True                                     ## do not check anything. just measure. overwrites AHP_CHECK_SPIKE_COUNT and AHP_CHECK_SPIKE_FREQ
AHP_DEBUG_FRAME=True                                    ## display ahp frame
AHP_VALID_COMBO=[(3,10),(3,20),(3,40),(3,60),(3,80),(3,100),(3,120),(3,200),(5,10),(5,20),(5,40),(5,60),(5,80),(5,100),(5,120),(5,200),(15,20),(15,50)]  ## used to generate the list of AHP output fields
AHP_SIMPLIFIED_PROTOCOL=True                            ## use new simplified protocol. same results, less bugs. requires correct protocol naming!
AHP_CORRECT_SIGNS=True                                  ## display true AHP as negative and ADP as positive. if False, signs are reversed

AHPV_FREQS=[200,133.33,100,66.66,50,20]                 ## for AHP protocol with varying freq, the frequency used
...
## process flags
PROCESS_PROTOCOLS=['foldername','iv','resistance','spontaneousactivity','timeconstant','ahp','ahpv','resonnance','ramp','sag']
...
IV_PROTOCOL_NAMES=['*IV_multiclamp*','3-IV curve','*CCSteps*','*IV*', '*02 IV_multiclamp *']
AHP_PROTOCOL_NAMES=['*5AP*','*5-AP*','*3AP*','*AHP*']
AHPV_PROTOCOL_NAMES=['*trains at different frequencies*']
TC_PROTOCOL_NAMES=['*[Tt]ime constant*',]
ZAP_PROTOCOL_NAMES=['*ZAP*', 'resonnance']
INPUTR_PROTOCOL_NAMES=['*resistance*']
SPONTANEOUS_PROTOCOL_NAMES=['*EPSP*', 'spontaneous', '3-Spontaneous', '*sponyaneous*']
